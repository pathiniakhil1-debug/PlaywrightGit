...
export class BuzzModulePage {
    constructor(page) {
        this.page = page;  // ▶ Store page reference 
        // ▶ Define locators
        this.buzzMenu = page.locator('//span[text()="Buzz"]');   // Buzz menu item
        this.postInput = page.locator('.oxd-buzz-post-input');   // Post input field
        this.postButton = page.locator('button[type="submit"]');  // Post button
        this.postedMessage = (message) => page.locator(`//p[text()='${message}']`).first();   // Locator for posted message
    }

    // ▶ Navigate to Buzz module
    async navigateToBuzz() {
        console.log("▶ Navigating to Buzz module");
        await this.buzzMenu.click();
    }

    // ▶ Post a message in Buzz
    async postMessage(message) {
        ...
        await expect(this.postedMessage(message)).toHaveText(message);
